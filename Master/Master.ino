...
void mqttCommandReceived(char* topic, byte* payload, unsigned int length) {
  ...
  //il flag messaggio e il flagRichiesta sono utilizzati in quanto il qos è 1, quindi per evitare che il messaggio venga stampato più di una volta
...
void mqttPublishScattaFoto() {
  ...
  //pubblico lo stato del bottone anche se off poiché così facendo all'accensione del client del raspberry questo non vadi a scattare in automatico la foto
